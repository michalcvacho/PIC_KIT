...
unsigned int SlavegetsI2C2(unsigned char * rdptr, unsigned int i2c2_data_wait)
{
    ...
    int wait = 0;
    unsigned char temp = I2C2RCV;        /* flush out old data already on I2CRCV to clear RBF flag */
    ...
    while(!I2C2STATbits.P)               /* check for stop bit */
    {
        ...
        *rdptr++ = I2C2RCV;              /* save byte received */
        ...
        I2C2CONbits.ACKDT = 0;      /* generate ACK sequence */
        I2C2CONbits.ACKEN = 1;
        while(I2C2CONbits.ACKEN == 1);  /* Wait till ACK sequence is over */
