 ...
 *
 *********************************************************************
 * FileName:        dma_chn_write_ev_enable_flags_lib.c
 * Dependencies:	Dma.h
 *
 * Processor:       PIC32
 *
...
{
	(_DmaMapTbl+chn)->intrClr=DMA_EV_ALL_EVNTS<<__DCHxINT_MASK_(CHERIE_POSITION);	// clear all events
	(_DmaMapTbl+chn)->intrSet=eFlags<<__DCHxINT_MASK_(CHERIE_POSITION);		// set the req event flags
}
